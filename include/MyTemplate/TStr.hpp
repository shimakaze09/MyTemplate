...
struct fixed_cstring {
  ...
  constexpr fixed_cstring(std::in_place_t, Chars... chars) noexcept
      : data{static_cast<value_type>(chars)..., 0} {
...
struct concat_helper {
 private:
  ...
  static constexpr auto get(std::index_sequence<LNs...>,
                            std::index_sequence<RNs...>) noexcept {
    return TStrC_of<Str0::View()[LNs]..., Str1::View()[RNs]...>{};
  }

 public:
  using type = decltype(get(std::make_index_sequence<Str0::Size()>{},
                            std::make_index_sequence<Str1::Size()>{}));
};
#else
