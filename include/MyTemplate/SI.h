...
struct SI;
template <typename IList, typename Impl>
using SI_t = typename SI<IList, Impl>::type;
...
struct SI
    : detail::SI_::SI_t<
          detail::SI_::TopoSort_t<detail::SI_::ImplTrait_IList_t<Impl>>, Impl> {
  ...
  static constexpr bool IsContain() noexcept {
    return TExistGenericity_v<
        IList, Instantiate_t<TypeList<detail::SI_::Nil, Impl>, Interface>>;
  }
};
}  // namespace My
...
struct TopoSortImpl<TemplateList<>, SortedIList> : IType<SortedIList> {};
...
struct TopoSortImpl<TemplateList<IHead, ITail...>, SortedIList> {
  ...
  struct Recursion<false>
      : IType<TPushFront_t<
            TopoSortImpl_t<InterfaceTrait_IList_t<IHead>, SortedIList>,
            IHead>> {};

  using type =
      TopoSortImpl_t<TemplateList<ITail...>,
