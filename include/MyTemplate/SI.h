...
using SIN =
    typename SI<TemplateList<>, TemplateList<TNBases...>>::template Ttype<Base>;

template <typename Base, typename ArgList,
          template <typename...> class... TNBases>
using SINT =
 ...
 private:
  ...
  struct Rec<false> {
    ...
    using NewTopoOrderBaseSet =
        typename TopoSort<TVBs, TopoOrderBaseSet, ArgList, CRTP>::type;
...
using SIICRTP =
    SIIPro<TopoSort_t<BaseList, TypeList<Args...>, true>, Impl, Args...>;
