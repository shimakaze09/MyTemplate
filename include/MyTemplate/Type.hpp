...
class NameID {
 public:
  ...
  constexpr bool Is(std::string_view str) const noexcept {
    return value == NameID{str}.GetValue();
  }

  explicit constexpr operator bool() const noexcept { return Valid(); }

  constexpr std::strong_ordering operator<=>(const NameID& rhs) const noexcept =
      default;
...
class TypeID {
 public:
  ...
  constexpr std::strong_ordering operator<=>(const TypeID& rhs) const noexcept =
      default;
...
class Name {
 public:
  ...
  constexpr Name(std::string_view str, NameID nameID) noexcept
      : str{str}, nameID{nameID} {
  ...
  constexpr bool Valid() const noexcept {
    return !str.empty() && nameID.Valid();
  }
  constexpr operator bool() const noexcept { return Valid(); }
  constexpr std::strong_ordering operator<=>(const Name& rhs) const noexcept {
    return nameID <=> rhs.nameID;
  }
...
class Type {
 public:
  ...
  constexpr Type(std::string_view str, TypeID typeID) noexcept
      : name{str, NameID{typeID.GetValue()}} {}
  ...
  constexpr TypeID GetID() const noexcept {
    return TypeID{name.GetID().GetValue()};
  }
  ...
  constexpr bool IsVoid() const noexcept {
    return type_name_is_void(name.GetView());
  }
  constexpr bool IsNullptr() const noexcept {
    return type_name_is_null_pointer(name.GetView());
  }
  constexpr bool IsIntegral() const noexcept {
    return type_name_is_integral(name.GetView());
  }
  constexpr bool IsFloatingPoint() const noexcept {
    return type_name_is_floating_point(name.GetView());
  }
  constexpr bool IsArray() const noexcept {
    return type_name_is_array(name.GetView());
  }
  constexpr bool IsEnum() const noexcept {
    return type_name_is_enum(name.GetView());
  }
  constexpr bool IsUnion() const noexcept {
    return type_name_is_union(name.GetView());
  }
  constexpr bool IsFunction() const noexcept {
    return type_name_is_function(name.GetView());
  }
  constexpr bool IsPointer() const noexcept {
    return type_name_is_pointer(name.GetView());
  }
  constexpr bool IsLValueReference() const noexcept {
    return type_name_is_lvalue_reference(name.GetView());
  }
  constexpr bool IsRValueReference() const noexcept {
    return type_name_is_rvalue_reference(name.GetView());
  }
  constexpr bool IsMemberPointer() const noexcept {
    return type_name_is_member_pointer(name.GetView());
  }

  // composite
  constexpr bool IsArithmetic() const noexcept {
    return type_name_is_arithmetic(name.GetView());
  }
  constexpr bool IsFundamental() const noexcept {
    return type_name_is_fundamental(name.GetView());
  }

  // properties

  constexpr bool IsConst() const noexcept {
    return type_name_is_const(name.GetView());
  }
  constexpr bool IsReadOnly() const noexcept {
    return type_name_is_read_only(name.GetView());
  }
  constexpr bool IsVolatile() const noexcept {
    return type_name_is_volatile(name.GetView());
  }
  constexpr bool IsCV() const noexcept {
    return type_name_is_cv(name.GetView());
  }
  constexpr bool IsReference() const noexcept {
    return type_name_is_reference(name.GetView());
  }
  constexpr bool IsSign() const noexcept {
    return type_name_is_signed(name.GetView());
  }
  constexpr bool IsUnsigned() const noexcept {
    return type_name_is_unsigned(name.GetView());
  }
  constexpr bool IsBoundedArray() const noexcept {
    return type_name_is_bounded_array(name.GetView());
  }
  constexpr bool IsUnboundedArray() const noexcept {
    return type_name_is_unbounded_array(name.GetView());
  }
  constexpr std::size_t Rank() const noexcept {
    return type_name_rank(name.GetView());
  }
  ...
  constexpr std::string_view Name_RemoveCV() const noexcept {
    return type_name_remove_cv(name.GetView());
  }
  constexpr std::string_view Name_RemoveConst() const noexcept {
    return type_name_remove_const(name.GetView());
  }
  constexpr std::string_view Name_RemoveTopMostVolatile() const noexcept {
    return type_name_remove_topmost_volatile(name.GetView());
  }
  constexpr std::string_view Name_RemoveLValueReference() const noexcept {
    return type_name_remove_lvalue_reference(name.GetView());
  }
  constexpr std::string_view Name_RemoveRValueReference() const noexcept {
    return type_name_remove_rvalue_reference(name.GetView());
  }
  constexpr std::string_view Name_RemoveReference() const noexcept {
    return type_name_remove_reference(name.GetView());
  }
  constexpr std::string_view Name_RemovePointer() const noexcept {
    return type_name_remove_pointer(name.GetView());
  }
  constexpr std::string_view Name_RemoveCVRef() const noexcept {
    return type_name_remove_cvref(name.GetView());
  }
  constexpr std::string_view Name_RemoveExtent() const noexcept {
    return type_name_remove_extent(name.GetView());
  }
  constexpr std::string_view Name_RemoveAllExtents() const noexcept {
  ...
  constexpr TypeID ID_AddConst() const noexcept {
    return TypeID{type_name_add_const_hash(name.GetView())};
  }
  constexpr TypeID ID_AddVolatile() const noexcept {
    return TypeID{type_name_add_volatile_hash(name.GetView())};
  }
  constexpr TypeID ID_AddCV() const noexcept {
    return TypeID{type_name_add_cv_hash(name.GetView())};
  }
  constexpr TypeID ID_AddLValueReference() const noexcept {
    return TypeID{type_name_add_lvalue_reference_hash(name.GetView())};
  }
  ...
  constexpr TypeID ID_AddLValueReferenceWeak() const noexcept {
    return TypeID{type_name_add_lvalue_reference_weak_hash(name.GetView())};
  }
  constexpr TypeID ID_AddRValueReference() const noexcept {
    return TypeID{type_name_add_rvalue_reference_hash(name.GetView())};
  }
  constexpr TypeID ID_AddPointer() const noexcept {
    return TypeID{type_name_add_pointer_hash(name.GetView())};
  }
  constexpr TypeID ID_AddConstLValueReference() const noexcept {
    return TypeID{type_name_add_const_lvalue_reference_hash(name.GetView())};
  }
  constexpr TypeID ID_AddConstRValueReference() const noexcept {
    return TypeID{type_name_add_const_rvalue_reference_hash(name.GetView())};
  }
