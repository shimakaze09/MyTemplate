...
struct Filter
    : Accumulate<List, AddIf<PushFront, Test>::template Ttype, TypeList<>> {};
...
struct QuickSort<TypeList<Head, Tail...>, Less> {
 private:
  ...
  using GEList =
      Filter_t<TypeList<Tail...>, Negate<LessThanHead>::template Ttype>;
...
template <typename List, template <typename...> class T>
struct ExistInstance<List, T, false, false>
    : ExistInstance<PopFront_t<List>, T, is_instance_of_v<Front_t<List>, T>> {};
